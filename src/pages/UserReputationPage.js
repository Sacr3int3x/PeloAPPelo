...
import { getUserReputation, getUserReviews } from "../services/transactions";
...
function UserReputationPage() {
  ...
  useEffect(() => {
    ...
    // Cargar tanto la reputación como las reseñas
    Promise.all([
      getUserReputation(userId, token),
      getUserReviews(userId, token),
    ])
      .then(([reputationData, reviewsData]) => {
        if (cancelled) return;
        setUserReputation(reputationData);
        setReputations(reviewsData);
        setError("");
      })
      .catch((error) => {
        if (cancelled) return;
  ...
  const stats = useMemo(() => {
    ...
    const total = userReputation.count || 0;
    const average = userReputation.average || 0;

    // Para las estadísticas detalladas, necesitamos calcular desde las reseñas individuales
    ...
    // Clasificar calificaciones desde las reseñas individuales
    ...
    return {
      ...
      percentages: {
        ...
        regular: total > 0 ? ((regular / total) * 100).toFixed(1) : 0,
        bad: total > 0 ? ((bad / total) * 100).toFixed(1) : 0,
      },
    };
  }, [userReputation, reputations]);
          ...
          <section className="reputation-list-section">
            ...
            {reputations && reputations.length > 0 ? (
              ...
              <div className="reputation-empty">
                ...
                <small>Este usuario aún no ha recibido calificaciones</small>
              </div>
            )}
          </section>
        </>
